...
describe('AdminPanel Feature', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (fetchVideos as any).mockResolvedValue(mockVideos);
  ...
  it('opens modal and adds new video', async () => {
    render(<AdminPanel />);

    const uploadBtn = screen.getByText('Upload Video');
    fireEvent.click(uploadBtn);

    expect(screen.getByText('Upload New Video')).toBeInTheDocument();

    // Fill Form
    fireEvent.change(screen.getByPlaceholderText('Enter video title'), { target: { value: 'New Admin Video' } });
    fireEvent.change(screen.getByPlaceholderText('Studio Name'), { target: { value: 'Studio X' } });

    // Submit
    // Find the button that says "Upload" inside the modal form
    // We can scope by modal container if needed, or just find the submit button.
    // The submit button has text "Upload" and type "submit".
    const submitBtns = screen.getAllByText('Upload');
    // The last one is likely the one in the modal (renders later).
    ...
    await waitFor(() => {
        expect(screen.getByText('New Admin Video')).toBeInTheDocument();
    });
  });
});
