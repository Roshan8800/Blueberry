    ...
    switch (currentView) {
            ...
            <div className="p-4 md:p-6 max-w-[1800px] mx-auto w-full animate-in fade-in duration-500">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                  {renderVideoList(MOCK_VIDEOS.filter(v => v.category === 'Trending'))}
                  {renderVideoList(MOCK_VIDEOS.slice(0,3))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
                  {renderVideoList(MOCK_VIDEOS.filter(v => v.category === 'Premium').concat(MOCK_VIDEOS.slice(2,5)).slice(0, 4))}
                </div>
              </div>
            </div>
        );
      ...
      case AppView.PREMIUM:
        return (
          <div className="p-6 max-w-[1800px] mx-auto animate-in fade-in duration-500">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
               {renderVideoList(MOCK_VIDEOS.filter(v => v.category === 'Premium' || v.tags.includes('exclusive') || v.rating > 95))}
               {renderVideoList(MOCK_VIDEOS.slice(0, 4))}
             </div>
          </div>
        );
      ...
      case AppView.MODEL_DETAIL:
        return selectedModel ? (
          <ModelProfile 
            model={selectedModel}
            videos={MOCK_VIDEOS.filter(v => Math.random() > 0.5)}
      ...
      case AppView.FAVORITES:
        const favoritesList = MOCK_VIDEOS.filter(v => favoriteIds.has(v.id));
        return (
           <div className="p-6 max-w-[1800px] mx-auto animate-in fade-in">
      ...
      case AppView.HISTORY:
        // For demo purposes, let's mock an empty history if needed, but currently using mock slice.
        // Let's assume if currentUser is guest, we might show empty history for demo.
        const historyList = MOCK_VIDEOS.slice(2, 6);
      ...
      case AppView.CATEGORY:
        const categoryVideos = MOCK_VIDEOS.filter(v => v.category === selectedCategoryName);
