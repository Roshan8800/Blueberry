...
import { shareVideo } from '../services/firebase';
...
const Player: React.FC<PlayerProps> = ({
  ...
  const suggested = videos.filter(v => v.id !== video.id);
  ...
  const handleShare = async (platform: string) => {
    if (!currentUser) {
      onShowToast("Please log in to share videos");
      return;
    }

    const result = await shareVideo(video.id, currentUser.id, platform);
    if (result.success) {
      onShowToast(`Shared to ${platform}!`);
    } else {
      onShowToast("Failed to share video");
    }
  ...
  const handleVideoEdit = (editedData: { startTime: number; endTime: number; cropArea?: { x: number; y: number; width: number; height: number } }) => {
    // In a real app, this would send to server for processing
    onShowToast("Video edit saved! Processing may take a few minutes.");
    console.log('Video edit data:', editedData);
  };

  // Premium Access Check
  const hasPremiumAccess = currentUser.plan === 'premium' || currentUser.plan === 'blueberry';
  const isPremiumContent = video.isPremium || false;
  const canAccessContent = !isPremiumContent || hasPremiumAccess;
  ...
  // Iframe handling
  const isIframeEmbed = video.embedUrl && (video.embedUrl.includes('<iframe') || video.embedUrl.includes('pornhub.com') || video.embedUrl.includes('youtube.com') || video.embedUrl.includes('vimeo.com'));
  const iframeSrc = isIframeEmbed && !video.embedUrl.includes('<iframe') ? video.embedUrl : video.embedUrl;
  // Use the embedUrl as source if it's a direct link (mp4, etc), otherwise no fallback
  const hasValidVideoSrc = video.embedUrl && !isIframeEmbed && video.embedUrl.startsWith('http');
  const videoSrc = hasValidVideoSrc ? video.embedUrl : null;
    ...
    <div className={`w-full mx-auto p-0 ${isTheaterMode ? 'max-w-full' : 'lg:max-w-[1800px] lg:p-6'} transition-all duration-500 relative z-20`}>
      ...
      <div className={`grid gap-6 ${isTheaterMode ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3 xl:grid-cols-4'}`}>
        ...
        <div className={`${isTheaterMode ? '' : 'lg:col-span-2 xl:col-span-3'} flex flex-col gap-4`}>
          ...
          >
             {isIframeEmbed ? (
                 ...
                 <video
                    ...
                    onError={() => onShowToast("Video failed to load. Please try again.")}
                    onClick={!isLocked ? togglePlay : undefined}
                    loop={isLooping}
                    playsInline
                 />
             ) : (
                 <div className="flex items-center justify-center h-full text-white text-xl">Video source not available</div>
             )}
          ...
          {!canAccessContent && (
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
              <div className="bg-dark-card p-8 rounded-2xl border border-gray-800 text-center max-w-md mx-4">
                <div className="w-16 h-16 bg-gradient-to-r from-yellow-500 to-yellow-400 rounded-full flex items-center justify-center mx-auto mb-4">
                  <i className="fa-solid fa-crown text-black text-2xl"></i>
                </div>
                <h3 className="text-xl font-bold text-white mb-2">Premium Content</h3>
                <p className="text-gray-400 mb-6">
                  This video requires a premium subscription to access. Upgrade now to unlock unlimited premium content.
                </p>
                <div className="space-y-3">
                  <button
                    onClick={() => onUpgradePrompt?.()}
                    className="w-full bg-gradient-to-r from-yellow-500 to-yellow-400 text-black font-bold py-3 rounded-lg hover:opacity-90 transition-opacity"
                  >
                    Upgrade to Premium
                  </button>
                  <button
                    onClick={() => setView?.(AppView.PRICING)}
                    className="w-full bg-gray-700 text-white py-2 rounded-lg hover:bg-gray-600 transition-colors text-sm"
                  >
                    View All Plans
                  </button>
                </div>
          ...
          <div className="bg-dark-card p-4 rounded-xl border border-gray-800">
              <div className="flex items-center gap-3 mb-2">
                ...
                {isPremiumContent && (
                  <span className="bg-gradient-to-r from-yellow-500 to-yellow-400 text-black text-xs font-bold px-2 py-1 rounded-full uppercase tracking-wider flex items-center gap-1">
              ...
              <div className="flex flex-wrap justify-between items-center gap-4">
                 ...
                 <div className="flex items-center gap-2 bg-black/20 p-1 rounded-full border border-gray-800/50">
                    ...
                    <div className="relative share-dropdown">
                      <button onClick={() => setShowShareOptions(!showShareOptions)} className="px-4 py-1.5 rounded-full hover:bg-gray-800 text-gray-300 hover:text-white transition-colors"><i className="fa-solid fa-share"></i></button>
                      {showShareOptions && (
                        <div className="absolute bottom-full right-0 mb-2 bg-black/90 border border-gray-800 rounded-xl p-2 w-48 shadow-xl animate-in fade-in slide-in-from-bottom-2 z-50">
                          <div className="space-y-1">
                            <button onClick={copyToClipboard} className="w-full text-left px-3 py-2 hover:bg-white/10 rounded text-sm text-white flex items-center gap-2">
                              <i className="fa-solid fa-link text-xs"></i> Copy Link
                            </button>
                            <button onClick={() => handleShare('twitter')} className="w-full text-left px-3 py-2 hover:bg-white/10 rounded text-sm text-white flex items-center gap-2">
                              <i className="fa-brands fa-twitter text-xs"></i> Share on Twitter
                            </button>
                            <button onClick={() => handleShare('facebook')} className="w-full text-left px-3 py-2 hover:bg-white/10 rounded text-sm text-white flex items-center gap-2">
                              <i className="fa-brands fa-facebook text-xs"></i> Share on Facebook
                            </button>
                            <button onClick={() => handleShare('reddit')} className="w-full text-left px-3 py-2 hover:bg-white/10 rounded text-sm text-white flex items-center gap-2">
                    ...
                    <div className="w-px h-4 bg-gray-700"></div>
                    <button
                      onClick={() => {/* TODO: Implement tipping */}}
                      className="px-4 py-1.5 rounded-full hover:bg-gray-800 text-gray-300 hover:text-red-400 transition-colors"
                      title="Tip Creator"
                    >
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 rounded-full hover:bg-gray-800 text-gray-300 hover:text-white transition-colors"
                      title="Edit Video"
                    >
              ...
              <div className="space-y-4">
                 {comments.length > 0 ? comments.map(c => (
                    ...
                    <EmptyView
                       ...
                       onAction={() => (document.querySelector('input[placeholder="Add a comment..."]') as HTMLInputElement)?.focus()}
                       isUltra={isUltra}
                    />
                 )}
              </div>
        ...
        <div className="lg:col-span-1 flex flex-col gap-3 h-full overflow-y-auto no-scrollbar">
           ...
           {suggested.length > 0 ? suggested.map((vid) => (
              <div
                key={vid.id}
              ...
              <EmptyView
                 ...
                 description="We couldn't find any related videos to suggest at this time."
                 actionLabel="Browse Videos"
                 onAction={() => setView?.(AppView.HOME)}
                 isUltra={isUltra}
              />
           )}
