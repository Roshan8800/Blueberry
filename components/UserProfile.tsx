...
import { User, AppView, Video } from '../types';
import { followUser, unfollowUser, isFollowing, getFollowers, getFollowing } from '../services/firebase';
...
interface UserProfileProps {
  ...
  videos: Video[];
  currentUser?: User | null; // For following functionality
}

import { updateUser } from '../services/firebase';

const UserProfile: React.FC<UserProfileProps> = ({ user, setView, onUpdateUser, videos, currentUser }) => {
  ...
  useEffect(() => {
    const checkFollowStatus = async () => {
      if (currentUser && currentUser.id !== user.id) {
        ...
        if (result.success) {
          setIsFollowingUser(result.data!);
        }
      }
    };
    checkFollowStatus();
  ...
  const handleSave = async () => {
      ...
      try {
        // Update Firestore
        await updateUser(user.id, { username: editName });
    ...
    <div className="p-6 max-w-7xl mx-auto animate-in fade-in duration-300">
        ...
        <div className="absolute -bottom-16 left-6 md:left-10 flex items-end gap-6">
          ...
          <div className="mb-4">
            {isEditing ? (
                ...
                {user.emailVerified && (
                  <span className="bg-green-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-1">
                    <i className="fa-solid fa-check"></i> Verified
                  </span>
                )}
                {user.twoFactorEnabled && (
                  <span className="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-lg flex items-center gap-1">
                    <i className="fa-solid fa-shield"></i> 2FA
                  </span>
                )}
                </h1>
            )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 pt-4">
        ...
        <div className="bg-dark-card p-5 rounded-xl border border-gray-800 text-center hover:border-gray-700 transition-colors">
           ...
           <p className="text-2xl font-bold text-white">{user.postsCount || 0}</p>
        ...
        <div className="mb-8 flex justify-center">
          ...
          >
            {isLoadingFollow ? (
              ...
              <i className={`fa-solid ${isFollowingUser ? 'fa-user-minus' : 'fa-user-plus'} mr-2`}></i>
            )}
