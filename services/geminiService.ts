 ...
 * Useful for the Admin panel when managing videos.
 */
...
/**
 * Uses Gemini to generate video recommendations based on user preferences or current video.
 * Returns an array of recommended video titles or tags.
 */
export const getVideoRecommendations = async (currentVideoTags: string[], userPreferences?: string[]): Promise<string[]> => {
  if (!apiKey) {
    throw new Error("Gemini API Key is required but not provided.");
  }

  try {
    const preferences = userPreferences ? userPreferences.join(', ') : 'general adult entertainment';
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Based on video tags: ${currentVideoTags.join(', ')} and user preferences: ${preferences}, suggest 5 related video categories or tags for recommendations. Return ONLY the comma-separated list of suggestions.`,
    });
    ...
    return currentVideoTags.slice(0, 5); // Fallback
  }
};